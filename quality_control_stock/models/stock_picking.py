...
from odoo.addons.quality_control_insp.models.qc_trigger_line import\
...
class StockPicking(models.Model):
    ...
    quality_insp_todo = fields.Boolean('Inspection Check', compute='_compute_insp')
    quality_insp_fail = fields.Boolean(compute='_compute_insp')
    
    
    def _compute_insp(self):
        for picking in self:
            todo = False
            fail = False
            for check in picking.move_lines:
                ...
                if fail and todo:
                    break
            picking.quality_insp_fail = fail
            picking.quality_insp_todo = todo
    ...
    def get_action_done(self):
        #res = super(models.Model).get_action_done()
