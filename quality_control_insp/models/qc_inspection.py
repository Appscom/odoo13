...
class QcInspection(models.Model):
    ...
    test = fields.Many2one(
        comodel_name='qc.test', string='Test', readonly=True,store=True)
    ...
    def action_confirm(self):
        for inspection in self:
            for line in inspection.inspection_lines:
                ...
                elif line.question_type == 'picture':
                    if not line.picture:
                        raise exceptions.UserError(
                            _("You should provide an image"))      
                else:
                    if not line.uom_id:
                        raise exceptions.UserError(
...
class QcInspectionLine(models.Model):
    ...
    @api.depends('question_type', 'uom_id', 'test_uom_id', 'max_value',
                 ...
                 'possible_ql_values','picture')
    def _compute_quality_test_check(self):
        for l in self:
            if l.question_type == 'qualitative':
                l.success = l.qualitative_value.ok
            elif l.question_type =='picture' and l.picture:
                l.success = l.is_image  
            elif l.question_type == 'quantitative':
    ...
    is_image = fields.Boolean(string="Is ok", store=True,default=True,invisible=True)
    
    
    # Add picture dependency
    @api.depends('picture')
    def _compute_result(self):
        super(QcInspectionLine, self)._compute_result()

    def _get_check_result(self):
        if self.question_type == 'picture' and self.picture:
            return _('Picture Uploaded')
        else:
            return super(QcInspectionLine, self)._get_check_result()
